...
// @homepageURL   https://github.com/Eichi76/musicbrainz-userscripts#musicbrainz-voice-actor-credits-eichi76-dev-version
// @downloadURL   https://raw.github.com/Eichi76/musicbrainz-userscripts/hoerspielforscher-voice-actor-experiments/dist/voiceActorCredits.user.js
// @updateURL     https://raw.github.com/Eichi76/musicbrainz-userscripts/hoerspielforscher-voice-actor-experiments/dist/voiceActorCredits.user.js
...
(function () {
	...
	function buildEditNote(...sections) {
		...
		if (typeof GM_info !== 'undefined') {
			sections.push(`${GM_info.script.name} (v${GM_info.script.version})`);
		}
...
#credit-parser label[title] {
	...
	async function initializeUI() {
		...
		addPatternInput({
			...
			defaultValue: /\s[–-]\s|:\s|\t+/, // typographische Varianten
		});
			...
			async (creditInput, event) => {
				...
				if (checkJSON(creditInput.value)) {
					...
					for (const line of credits) {
						if (line.importUrl) {
							parsedLines.unshift(`Die folgenden Credits wurden von ${line.importUrl} importiert`);
							continue;
						}
						// bergebe das Credit Object
						const parserStatus = await parser(line, event);
						// Wenn Credit 'done' ist...
						if (parserStatus !== 'skipped') {
							// ...fge den Job + Knstler + Rollennamen als String ins Parsed Array
							parsedLines.push(
								`${line.linktype}: ${line.name}${
								line.attributesTypes[0]?.text ? ' - ' + line.attributesTypes[0].text : ''
							}`
							);
						}
						// Wenn Credit bersprungen wurde...
						if (parserStatus !== 'done') {
							// ...fge das Objekt ins Skipped Array
							skippedLines.push(line);
						}
					}
				} else {
					// ...ansonsten fhre Kellnerds Code aus
				...
				if (removeParsedLines.checked) {
					// Wenn es bersprungene Credozs als Objekte gibt
	...
	function addPatternInput(config) {
		...
		const patternInput = createElement(
			`<input type="text" class="pattern" name="${id}" id="${id}" placeholder="String or /RegExp/" />`
		);
		...
		const resetButton = createElement(
			`<button type="button" title="Reset the input to its default value">Reset</button>`
		);
		...
		patternInput.addEventListener('change', function () {
			explanationLink.href =
				'https://kellnerd.github.io/regexper/#' + encodeURIComponent(getPatternAsRegExp(this.value) ?? this.value);
		...
		container.insertAdjacentHTML(
			'beforeend',
			`<label for="${id}" title="${config.description}">${config.label}:</label>`
		);
	 ...
	 */
	const callAPI = rateLimit(fetch, 60 * 1000);
...
<div id="credit-import-tools">
	...
	function buildVoiceActorCreditParserUI() {
		...
		addParserButton(
			'Parse voice actor credits',
			async (creditLine, event) => {
				...
				if (discogsURL) {
					...
					const messages = newMatches.map((match) =>
						[
							...
							`<a href="${seedURLForEntity(
							'artist',
							match.MBID,
							match.externalURL,
							180,
							artistSeedNote
						)}" target="_blank">${match.name}</a>`,
					...
					messages.unshift(
						`Successfully imported ${importedCredits} of ${result.totalCredits} credits, ${result.mappedCredits} of them were mapped automatically.`
					);

					dom('credit-import-status').innerHTML = messages.map((message) => `<p>${message}</p>`).join('\n');
				}
			},
			['Import credits from Discogs', 'SHIFT key to ignore an existing URL relationship and prompt for an URL'].join('\n')
		);
	}
